...
AFRAME.registerComponent('model-viewer', {
  ...
  onModelLoaded: function (evt) {
    let absoluteCenter = this.centerAndScaleModel();
    ...
    try {
        ...
        document.getElementById('index-container').append(apiFrame);

        let iframe = document.getElementById('api-frame');
        let client = new Sketchfab('1.12.1', iframe);

        let modelUID = 'ea10ab80836848b3abe522dcee76e394';  // get model ID from json in future updates
                ...
                await api.addEventListener('viewerready', async function () {
                    ...
                    await api.getAnnotationList(async function (err, annotations) {
                        if (!err) {
                            ...
                            // Calcolare il centro e la scala del modello
                            modelEl.object3D.updateMatrixWorld();
                            let box = new THREE.Box3().setFromObject(gltfObject);
                            let center = box.getCenter(new THREE.Vector3());
                            let size = box.getSize(new THREE.Vector3());

                            console.log(center);

                            // Ridimensionare proporzionalmente
                            let scaleFactor = Math.min(1.6 / size.y, 2.0 / size.x, 2.0 / size.z);

                            // Centrare e scalare le annotazioni
                            annotations.forEach(function (annotation, index) {
                                console.log('Annotation #' + index + ' in position:', annotation.position);
                                gltfObject.traverse(function (node) {
                                    if (node.isMesh && node.geometry) {
                                        ...
                                        // Usa lastComputedWorldPosition per ottenere la posizione dell'annotazione
                                        let annotationVec = new THREE.Vector3(
                                            annotation.lastComputedWorldPosition[0],
                                            annotation.lastComputedWorldPosition[1],
                                            annotation.lastComputedWorldPosition[2]
                                        );

                                        let annotationPosition = new THREE.Vector3(
                                            annotation.barycentricWeights[0] * annotationVec.x,
                                            annotation.barycentricWeights[0] * annotationVec.y,
                                            annotation.barycentricWeights[0] * annotationVec.z
                                        );
                                        ...
                                        console.log(absoluteCenter, eye, annotationPosition, annotationVec);

                                        // Centrare l'annotazione rispetto al modello e al punto di vista
                                        //annotationPosition.sub(absoluteCenter.sub(eye));
                                        annotationVec.x += 0.36;
                                        annotationVec.y += 0.225;
                                        annotationVec.z -= 0.55;
                                        let transformedVec = annotationVec.multiplyScalar(scaleFactor);
                                        console.log('Transformed annotationVec:', transformedVec);

                                        // Definire la posizione dinamica per l'annotazione
                                        let dynamicPosition = `${transformedVec.x} ${transformedVec.z} ${-transformedVec.y}`;
                                        console.log('Dynamic position:', dynamicPosition);

                                        let color = 'purple';
                                        if (index === 1) color = 'orange';
                                        else if (index === 2) color = 'white';
                                        else if (index === 3) color = 'brown';

                                        // Creare una sfera per l'annotazione
                                        let entity = document.createElement('a-sphere');
                                        entity.setAttribute('class', 'annotations');
                                        entity.setAttribute('id', 'annotation-' + index);
                                        entity.setAttribute('event-set__enter', '_event: mouseenter; color: black');
                                        entity.setAttribute('event-set__leave', '_event: mouseleave; color: lightgrey');
                                        entity.setAttribute('event-set__click', '_target: #annotation-text-' + index + '; visible: true; color: grey');
                                        entity.setAttribute('radius', 0.01 * scaleFactor); // Scala la sfera in base al modello
                                        entity.setAttribute('color', color);
                                        entity.setAttribute('position', dynamicPosition);
                                        entity.classList.add('raycastable');

                                        modelEl.appendChild(entity);

                                        // Creare il testo per l'annotazione
                                        let text = document.createElement('a-text');
                                        text.setAttribute('id', 'annotation-text-' + index);
                                        text.setAttribute('class', 'annotations-text');
                                        text.setAttribute('align', 'center');
                                        text.setAttribute('color', '#FFF');
                                        text.setAttribute('visible', 'false');
                                        text.setAttribute('scale', `${scaleFactor} ${scaleFactor} ${scaleFactor}`);
                                        text.setAttribute('position', dynamicPosition);
                                        text.setAttribute('geometry', 'primitive: plane; width: 3.2; height: 0.5');
                                        text.setAttribute('material', 'color: #ccff99');
                                        text.setAttribute('event-set__enter', '_event: mouseenter; color: grey');
                                        text.setAttribute('event-set__leave', '_event: mouseleave; color: white');

                                        modelEl.appendChild(text);
                                    };
                                });
                            });
                        } else {
                            console.error('Error during annotation retrieval:', err);
                        }
                    });
                });
            },
            error: function () {
                console.error('Error during the loading of the Sketchfab model.');
            }
        });
  ...
  centerAndScaleModel: function () {
    ...
    console.log("centerandscalemodel: ", center);
    size = box.getSize(new THREE.Vector3());
