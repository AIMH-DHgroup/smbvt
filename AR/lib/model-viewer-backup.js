...
AFRAME.registerComponent('model-viewer', {
  ...
  centerAndScaleModel: function () {
    ...
    try {
        ...
        let thisEl = this.el; // riferimento all'entità principale
                    ...
                    await api.getAnnotationList(async function (err, annotations) {
                        if (!err) {
                            ...
                            annotations.forEach(function (annotation, index) {
                                ...
                                // Centrare l'annotazione rispetto al modello
                                //annotationVec.sub(center);
                                annotationVec.sub(eye);
                                annotationVector[parseInt(index)] = annotationVec.multiplyScalar(scaleFactor);

                                console.log('Transformed annotationVec:', annotationVector[parseInt(index)]);

                                // Definire la posizione dinamica per l'annotazione
                                let dynamicPosition = `${annotationVector[index].x} ${-annotationVector[index].y} 0`;
                                ...
                                entity.setAttribute('scroll-to-zoom', '');

                                thisEl.appendChild(entity);
                                ...
                                text.setAttribute('scroll-to-zoom', '');

                                thisEl.appendChild(text);
                            });
                        } else {
