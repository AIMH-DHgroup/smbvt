                    ...
                    await api.getAnnotationList(async function (err, annotations) {
                        if (!err) {
                            ...
                            annotations.forEach(function (annotation, index) {
                                ...
                                let dynamicPosition = `${transformedVec.x} ${transformedVec.y} ${transformedVec.z}`;
                                ...
                                entity.setAttribute('radius', 0.05 * scaleFactor); // Scala la sfera in base al modello
                                entity.setAttribute('color', 'lightgrey');
                                entity.setAttribute('position', dynamicPosition);
                                entity.classList.add('raycastable');

                                // Cerca il nodo corrispondente alla geometria
                                gltfObject.traverse(function (node) {
                                    if (node.isMesh && node.uuid === annotation.geometryUid) {
                                        console.log('Found matching mesh for annotation:', node.name);

                                        // Posiziona l'annotazione rispetto alla mesh
                                        let annotationPosition = new THREE.Vector3();
                                        annotationPosition.setFromMatrixPosition(node.matrixWorld);

                                        entity.setAttribute('position', `${annotationPosition.x} ${annotationPosition.y} ${annotationPosition.z}`);
                                    }
                                });
