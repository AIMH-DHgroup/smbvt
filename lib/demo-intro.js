		...
		success: function(resp) {
			...
			if(resp.username != ""){
				...
				if( resp.username != resp.usernameToDisplayInMenu){
					...
					var toolHtmlPageName= "tool2.html"
				
				}	else {
				
					var toolHtmlPageName= "tool.html"
				}					
				...
				for(var i = 0; i < resp.jsonData.length; i ++) {
					...
					htmlTable += "<tr>"
					htmlTable += "<td style='width:10%'>"+resp.jsonData[i][0]+"</td> <td style='width:80%'><a href='"+toolHtmlPageName+"?idwiki=" + resp.jsonData[i][2].toUpperCase() + "&idnar="+resp.jsonData[i][0]+"'>" + resp.jsonData[i][1] + "</a></td>"
					...
					$("#narratives-menu").append("<div class='narra-list-item' id='storyMenu"+i+"'><a href='tool.html?idwiki=" + resp.jsonData[i][2].toUpperCase() + "&idnar="+resp.jsonData[i][0]+"' class='data' target = '_blank'>" + resp.jsonData[i][1] + "</a><div class='deleteButton' id='"+resp.jsonData[i][0]+"-"+resp.jsonData[i][2]+"-"+resp.username+"-"+escapeTrash+"' onclick='deleteNarrative(this.id)'> <b class='x'>×</b></div></div>");
		...
		success: function(resp) {
				...
				htmlTable += "<thead> <tr> <th>Id</th> <th>Title</th> <th>Delete</th> </tr> </thead> <tbody>"
 				for(var i = 0; i < resp.jsonData.length; i ++) {
					...
					htmlTable += "<tr>"
					htmlTable += "<td style='width:10%'>"+resp.jsonData[i][0]+"</td> <td style='width:80%'><a href='tool.html?idwiki=" + resp.jsonData[i][2].toUpperCase() + "&idnar="+resp.jsonData[i][0]+"'>" + resp.jsonData[i][1] + "</a></td>"
