...
/* 			var DBsToLoad = [];
               ...
               for(var i = 0; i < resp.allDBName.length; i ++) {
				...
				for(var i = 0; i < resp.allDBName.length; i ++) {
					...
					$("#narratives-menu").append("<div class='narra-list-item "+classMenu+"' id='storyMenu"+i+"'><a href='toolDemo.html?idwiki=" + resp.allDBName[i][2].toUpperCase() + "&idnar="+resp.allDBName[i][0]+"' class='data' target = '_blank'>" + resp.allDBName[i][1] + "</a><div class='deleteButton' id='"+resp.allDBName[i][0]+"-"+resp.allDBName[i][2]+"-"+resp.username+"-"+resp.allDBName[i][1]+"' onclick='deleteNarrative(this.id)'> <b class='x'>×</b></div></div>");
        ...
        if (err.name === 'not_found') {
          ...
          for (var narraID in response.narratives) {
              var narrative = response.narratives[narraID];
                $("#narratives-menu").append("<li class='narra-list-item'><a href='toolDemo.html?" + narrative.qid + "' class='data' style='background-color: " + getColor(narrative.type) + "' target = '_blank'>" + narrative.name + "</a></li>");
          };
      }
    });
}
    ...
    $.getJSON(sparqlURL)
		.done(function (data) {
			...
			// else if sparql query is empty...
				...
				window.history.pushState('index', '', '/demo.html');
	...
	$.ajax({
		...
		success: function(resp) {
			...
			console.log(resp.output)
				...
				var searchOtherNarratives="<br/><br/>Find all stored narratives: <input type='text' value='https://tool.dlnarratives.eu/Search/?dataset=prova2' readonly>"
			
			
		    showModal(
				"Narrative published",
				"Your narrative has been stored in the Fuseki Triplestore.<br/><br/>Try official SMBVT to create a Story map web page<br/><br/>"+searchOtherNarratives,
				"back",
				"ok",
				function() {
...
/*             narra.remote.logout().then(function () {
			...
			$.ajax({
				...
				success: function(resp) {
					console.log(resp.msg)
					console.log(currentTime() + "Successfully logged out");
					window.open("https://tool.dlnarratives.eu/demo.html","_self");
				},
				error: function(response){
					var a= JSON.stringify(response)
					//alert(a)
...
function dbNameRequest(qids, title) {
    ...
    $.getJSON(sparqlURL, function (data) {    
        ...
        for (var i=0; i < data.length; i++) {
            ...
            if (!(qid in added)) {
                $("#narratives-menu").append("<li class='narra-list-item'><a href='toolDemo.html?" + qid + "' class='data' target = '_blank'>" + titlecase(data[i].enName.value) + "</a></li>");
                added[qid] = {};
        }
        };
    });
